...
func (s Scaffolder) ProjectExample() string {
  ...
  COMMAND_NAME init --project-version=1 --domain example.org --license apache2 --owner "The Kubernetes authors"
`
...
func (s *Scaffolder) BindProjectFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&s.dep, "dep", true, "if specified, determines whether dep will be used.")
	s.depFlag = fs.Lookup("dep")
	fs.StringArrayVar(&s.depArgs, "depArgs", nil, "Additional arguments for dep")
